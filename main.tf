...
resource "yandex_compute_instance" "bastion-host" {
  ...
  network_interface {
    ...
    nat                = true
    security_group_ids = [yandex_vpc_security_group.user-gw.id]
...
resource "yandex_alb_load_balancer" "alb1" {
  name               = "alb1"
  network_id         = yandex_vpc_network.network1.id
  security_group_ids = [yandex_vpc_security_group.user-balancer.id]
...
resource "yandex_vpc_security_group" "user-balancer" {
  name       = "user-balancer"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "user-private" {
  name       = "user-private"
  network_id = yandex_vpc_network.network1.id
  ...
  ingress {
    protocol          = "ANY"
    description       = "any"
    security_group_id = yandex_vpc_security_group.user-gw.id
  }
...
resource "yandex_vpc_security_group" "user-public" {
  name       = "user-public"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "user-gw" {
  name       = "user-gw"
  network_id = yandex_vpc_network.network1.id
